...
////intelligent browser
void TabWidget::my_moveTab(int fromIndex, int toIndex) //탭이동
{
...
/*
 void TabWidget::load_hiddenTab(Graph* head, char* cur_url)
 1.탭 숨김 및 히든탭 로딩
...
void TabWidget::load_hiddenTab(Graph* head, char* cur_url)
{
	...
	if(f_node_index>=0)
	{
		...
		int num_edge = f_node->get_e_index();

	   	int k=0;	
		for (int i =0 ; i < num_edge; ++i) {	
			if(strcmp(f_node->get_edge()[i]->url,cur_url)==0)
			{
              			//cout<<"same url"<<endl;
           		}
           		else{
            			set_preurl(k,f_node->get_edge()[i]->url);
            
               			webView(k/*+1*/)->loadUrl(QUrl(f_node->get_edge()[i]->url));
               			k++;
           		}
...
void TabWidget::webViewLoadStarted()
{
    ...
    if (-1 != index) {
        ...
        setTabIcon(index, icon);///////////////tab clear/////
    }
}
...
{
    //cout<<"webViewTitleChanged"<<endl;
    WebView *webView = qobject_cast<WebView*>(sender());
    int index = webViewIndex(webView);
    if (-1 != index) {
        setTabText(index, title);///////////////tab clear/////
    }
    if (currentIndex() == index){
	if(webView->check_flag==0 && check_title != title){		
		//webView->my_linkclicked(webView->url());
//cout<<"~~~in webViewtitlechanged "<<title.toUtf8().data()<<endl;
	}
        check_title = title;
        
        emit setCurrentTitle(title);///////////////tab clear/////
    }
