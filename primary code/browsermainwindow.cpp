...
//intelligent browser start 
   ...
   if(m_tabWidget->count()<5)  ////브라우저 시작 시 탭 생성
   {
...
printf(" ---------------------------------------------------------------\n");
	    printf("\ncreate tab time : %ld msecs %ld nsecs \n\n",msecs , nsecs);
printf(" ---------------------------------------------------------------\n");
...
{
    //cout<<"BrowserMainwindow.cpp slotUpdateWindowTitle **********************************"<<endl;
if(title !=QString()){
   // cout<<"title : " <<title.toUtf8().data()<<endl;
    //cout<<"url : " <<currentTab()->url().toString().toUtf8().data()<<endl;
    //currentTab()->my_linkclicked(currentTab()->url());
}
    if (title.isEmpty()) {
...
void BrowserMainWindow::slotHome()
{
    ...
    QString home = settings.value(QLatin1String("home"), QLatin1String("http://www.naver.com/")).toString();
    loadPage(home);
}
...
void BrowserMainWindow::loadPage(const QString &page)
{
    ...
    //cout<<"browseMainwindow.cpp loadpage()~~~~~~~~~~~~~~"<<url.toString().toUtf8().data()<<"asafasfasfAfasfaf"<<endl;
