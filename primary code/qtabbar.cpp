...
///////////////////////////////////////////intelligent browser start
    int QTabBar::get_HiddenIndex()//not used
	{
		Q_D(QTabBar);
		return d->getHiddenIndex();
	}	
/*
...
//////////////////////////////////////intelligent browser start
			if(tabList[i].hidden)		//tabs having active hidden flag are not allocated in tabbar
				continue;
///////////////////////////////////////intelligent browser end
        ...
        for (i = 0; i < tabList.count(); ++i, ++tabChainIndex) {
//////////////////////////////////////intelligent browser start
			if(tabList[i].hidden) //464,465 line
				continue;
///////////////////////////////////////intelligent browser end
...
///////////////////////////////////////////////////////////////intelligent browser start
/*!
    Hide the tab at position \a index.
*/

void QTabBar::set_tabHidden(int index, bool hide)//not used
{
	Q_D(QTabBar);		
	d->tabList[index].hidden = hide;
}

void QTabBar::tabHide(int index, int to)
{

	Q_D(QTabBar);
#ifndef INIT_HIDE// Hide in execution stage
#define INIT_HIDE
//	if(index<0){
//	moveTab(0, 3);
//	for(int i=0 ; i<5 ; i++) {//hide 0 ~ 5th tabs by setting Rect_size to zero
//		d->tabList[i].hidden = true;
//		d->tabList[i].rect.setRect(0, 0, 0, 0);
//		d->tabList[i].maxRect= QRect(0, 0, 0, 0);
//		d->tabList[i].minRect= QRect(0, 0, 0, 0);
//	}
//	d->layoutTabs();//renew the tabbar
//	return;
//	}
#endif
...
printf("\n\n%%%%%%%%%%%%%%%%%%%%%%%%\nnow %dth tab is hidden\n%%%%%%%%%%%%%%%%%%\n\n", index);
/*	for (i = 0; i < tabList.count(); ++i) {
            QSize sz = d->tabSizeHint(i);
//            tabList[i].maxRect = QRect(x, 0, sz.width(), sz.height());
            x += sz.width();
  //          maxHeight = qMax(maxHeight, sz.height());
            sz = q->minimumTabSizeHint(i);
    //        tabList[i].minRect = QRect(minx, 0, sz.width(), sz.height());
		...
		d->tabList[index].hidden = true;//hide the tab by setting Rect_size to zero
		d->tabList[index].rect.setRect(0, 0, 0, 0);
		d->tabList[index].maxRect= QRect(0, 0, 0, 0);
		d->tabList[index].minRect= QRect(0, 0, 0, 0);
	//		d->layoutWidgets();
//		d->tabLayoutChange();
...
void QTabBar::tabShow(int index)//set hidden flag of the tab to false
{
	Q_D(QTabBar);

    if (d->validIndex(index))
	{
		d->tabList[index].hidden = false;
//		d->layoutTabs();
//	   	d->refresh();
//		update();
	}
...
/*	if(index == 2)
    ...
    if (d->validIndex(index) && d->currentIndex != index) {
        ...
        update();   
printf("\nasd\n *_*_*_*_*_* QTabBar setCurrentIndex idx= %d *_*_*_*_*_* \nasd\n", index);
